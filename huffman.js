...
class HuffmanCoder{
    ...
    stringify(node){
        // Encoding Huffman tree (inorder)
        if(typeof(node[1])==="string"){
            return '\''+node[1]; // to differntiate diff characters
        }
    ...
    display(node, modify, index=1){
        if(modify){ // preorder(modify helps in printing both compressed and decompressed)
            node = ['',node];
            if(node[1].length===1)
                node[1] = node[1][0];
        }
    ...
    encode(data){
        ...
        // inserting Elements in Max Heap with negative sign to act as Min heap
        for(const key in mp){
        ...
        // mapping character to binary string
        let binary_string = "";
        for(let i=0;i<data.length;i++) {
        ...
        //Binary string to corresponding chracter array
        let result = "";
